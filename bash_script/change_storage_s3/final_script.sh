...
time_before_1_days_in_seconds=$(date -d '24 hour ago' '+%s') &&
...
log_group_name=<give_your_log_group_name_here>
log_stream_name_prefix=<give_your_log_stream_name_prefix_here>
...
# filter and store the JSON output in a another file named output_initial.json -
 aws logs filter-log-events --log-group-name $log_group_name --log-stream-name-prefix $log_stream_name_prefix --start-time $start_time --end-time $end_time --filter-pattern ["eventName"="*GetObject*"] > output_initial.json

# cut backslashes, extra quation from optput_initial.json and store it to output_final.json -
 cat output_initial.json | awk '{gsub(/\\/,"")}1' | awk '{gsub(/"{/,"{")}1' | awk '{gsub(/}"/,"}")}1' > output_final.json

# pick up file name which has been accessed within 7 days and store those in a variable file named file_path.txt -
jq -r '.events[].message.requestParameters.key' output_final.json > file_path_initial.txt


awk '!seen[$0]++' file_path_initial.txt > file_path_final.txt
bucket_name=<give_your_bucket_name_here>
for item in `cat file_path_final.txt`
do
