...
echo
echo "showing current rolebindings on this cluster before apply"
echo
...
do
   ...
   # check is the rolebinding already exists or it is a new rolebinding and showing the yaml 
   # rb_existance_check=$(kubectl get rolebindings --namespace=$ns_name | awk '{print $1}' | grep "\<$rb_name\>")
   # if [ "$rb_existance_check" == "$rb_name" ]; then 
    # echo "The rolebinding name $rb_name is already exist and showing the yaml file"
    # echo
    kubectl get -o yaml rolebindings $rb_name -n $ns_name 
    # else
